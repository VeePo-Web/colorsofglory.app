...
import { reportPath, type ScenarioSummary } from "./metrics.js";
...
function runPreflight(): boolean {
  ...
  writeFileSync(
    reportPath("preflight-ok.md"),
    `# OTP Preflight OK\n\nNo SMS sends were made.\n\nGenerated: ${new Date().toISOString()}\n`,
  );
  return true;
}

function runScenario(script: string): number {
...
function loadScenarioSummary(name: string): ScenarioSummary | null {
  const path = reportPath(`${name}.json`);
  if (!existsSync(path)) return null;
  return JSON.parse(readFileSync(path, "utf8")) as ScenarioSummary;
}

function collectErrorCounts(summaries: ScenarioSummary[]): Array<[string, number]> {
...
function claimRows(summaries: ScenarioSummary[]): string[] {
  const byClaim = new Map<string, ScenarioSummary[]>();
  for (const summary of summaries) {
