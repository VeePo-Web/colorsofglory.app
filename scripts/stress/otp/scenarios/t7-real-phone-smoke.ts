...
export async function runT7(): Promise<void> {
  ...
  if (!getBooleanEnv("REAL_PHONE_OPT_IN") || !phone.startsWith("+")) {
    const summary = writeSkippedScenario(
      SCENARIO,
      ["C1", "C3"],
      "REAL_PHONE_OPT_IN=1 and REAL_PHONE_E164 are required",
    );
  ...
  const status: ScenarioStatus = results.length === 2 && results.every((result) => result.accepted) ? "GREEN" : "RED";
  const summary = writeScenarioSummary(
    ...
    ["C1", "C3"],
    status,
    `${results.filter((result) => result.accepted).length}/${results.length} real-phone steps accepted`,
  );
  printSummary(summary);
}
