...
// ── R30 (transcript → sheet-section lines) lived here until 2026-08 with
// ZERO UI callers — dead code that read as a live path. Deleted (hallway
// ledger F10); the DB RPCs `take_transcript_lines` /
// `apply_transcript_to_section` remain server-side, and this client seam is
// one `git log -S fetchTranscriptLines` away when the song-sheet lane's UI
// moment for it actually arrives.
