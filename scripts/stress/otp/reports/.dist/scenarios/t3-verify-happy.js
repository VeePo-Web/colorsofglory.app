...
export async function runT3() {
    ...
    const status = accepted === results.length && p95 <= 800 ? "GREEN" : "RED";
    const summary = writeScenarioSummary(SCENARIO, startedAt, results, ["C3"], status, `${accepted}/${results.length} sessions, p95 ${p95}ms`);
