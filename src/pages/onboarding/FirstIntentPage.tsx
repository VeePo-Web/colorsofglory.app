...
const IntentCard = ({ icon: Icon, title, description, onClick, accent }: IntentCardProps) => (
  <button
    ...
    style={{
      ...
      border: accent ? "1.5px solid var(--cog-gold)" : "1.5px solid var(--cog-border)",
    ...
    <div className="flex items-start gap-4">
      <div
        ...
        style={{
          width: 44,
          height: 44,
          backgroundColor: accent ? "var(--cog-gold-a12)" : "rgba(0,0,0,0.04)",
        }}
      >
        <Icon
          size={20}
          strokeWidth={1.6}
          style={{ color: accent ? "var(--cog-gold)" : "var(--cog-warm-gray)" }}
        />
      </div>
      <div className="pt-0.5">
        ...
        <p className="text-[0.875rem] leading-relaxed" style={{ color: "var(--cog-warm-gray)" }}>
          {description}
        </p>
      </div>
    </div>
...
const FirstIntentPage = () => {
  ...
  return (
    <OnboardingShell>
      ...
      {/* Microcopy */}
      <p className="text-[13px] text-center" style={{ color: "var(--cog-muted)" }}>
