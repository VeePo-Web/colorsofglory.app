...
const STATUS: Record<string, number> = {
  ...
  FORBIDDEN: 403,
  SONG_NOT_FOUND: 404,
  NEW_OWNER_NOT_MEMBER: 409,
  TRANSFER_BLOCKED_QUOTA: 402,
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (typeof song_id !== "string" || typeof new_owner_user_id !== "string") {
      return jsonResponse({ ok: false, code: "INVALID_INPUT", message: "song_id and new_owner_user_id required" }, 400);
    }

    const admin = adminClient();
    const { data, error } = await admin.rpc("safe_transfer_song_owner", {
      _song_id: song_id, _new_owner: new_owner_user_id, _actor: user.id,
    });
    if (error) return jsonResponse({ ok: false, code: "INTERNAL", message: error.message }, 500);
    const code = (data as string) ?? "INTERNAL";
    if (code === "OK" || code === "NOOP") return jsonResponse({ ok: true, code }, 200);
