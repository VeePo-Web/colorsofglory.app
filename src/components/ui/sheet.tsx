...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-background p-6 shadow-[0_25px_60px_-12px_hsl(var(--foreground)/0.15)] transition ease-cog data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title ref={ref} className={cn("font-serif text-lg font-light text-foreground", className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description ref={ref} className={cn("text-[13px] font-sans text-muted-foreground leading-relaxed", className)} {...props} />
));
