...
**Published by:** E3 · **Consumed by:** A2 (type), A3 (data layer), E1 (gating), E2 (activity), D2/D3 (the non-destructive ethos this makes literal)
...
### The snapshot payload (v1)

```jsonc
{
  "v": 1,
  "song":  { "title": "Colors of Glory" },   // string | null
  "sheet": { /* SheetDoc */ }                 // or null = the song before its first words
}
```

`sheet` is the full `SheetDoc` from `src/lib/chords/sheetState.ts` — sections → lines →
key-independent NumberChord anchors, plus key/mode/capo/bpm/display meta. It is the complete
restorable song state and round-trips losslessly through `saveSongSheet`.
...
previewed" card — **never raw JSON, never a crash**.

## 2. The A3 data API (filed and implemented — `src/integrations/cog/versions.ts`)

Same pattern as the notes seam: this module is the **only** place that queries `song_versions`.
No component, hook, or page touches the table. Pure helpers (`parseSnapshot`,
...
| `captureCurrentState(songId)` | Current `{title, sheet}` as a v1 snapshot (reads via `getSongSheet` — the sheet's own A3 seam, never other lanes' tables). |
...
2. **Apply.** Write the target snapshot's sheet through `saveSongSheet` (the C3/A3 sheet seam).
   A sheetless target restores to a blank sheet — the pre-restore version still holds
   everything.
...
`viewer`/non-member → read-only timeline. When E1 ships `useCapabilities`, collapse this hook to
consume it; components only ever see `{ canSave, canRestore }`. The server (RLS) remains the
...
- **E2:** add `version_saved` / `version_restored` to `SongActivityKind` + an ingest path; then
  fill the emit in `versions.ts`.
- **E1:** ship `useCapabilities`; collapse `useVersionCapabilities` onto it.
- **Lovable (only if product wants title restore):** applying a snapshot restores the *sheet*;
  the stored `song.title` is displayed but not written back (no `updateSong` seam exists yet).
