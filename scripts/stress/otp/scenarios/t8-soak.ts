...
export async function runT8(): Promise<void> {
  ...
  if (!budget.canReserve(totalSends)) {
    const summary = writeSkippedScenario(
      SCENARIO,
      ["C1", "C2"],
      `remaining budget ${budget.remaining} is below required ${totalSends}`,
    );
  ...
  for (let i = 0; i < totalSends; i += 1) {
    ...
    if (i < totalSends - 1) await sleep(30_000);
  }
  ...
  const status: ScenarioStatus = successRate >= 0.98 && p95 <= 5000 ? "GREEN" : "YELLOW";
  const summary = writeScenarioSummary(
    ...
    ["C1", "C2"],
    status,
    `${(successRate * 100).toFixed(1)}% accepted over ${minutes} minutes, p95 ${p95}ms`,
  );
  printSummary(summary);
}
