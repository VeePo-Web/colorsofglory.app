...
export async function runT4(): Promise<void> {
  ...
  const status: ScenarioStatus = falseSessions === 0 ? "GREEN" : "RED";
  const summary = writeScenarioSummary(
    ...
    ["C4"],
    status,
    `${falseSessions} false sessions issued across ${results.length} negative verifications`,
  );
