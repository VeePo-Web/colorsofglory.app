...
export async function runT2(): Promise<void> {
  ...
  if (numbers.length < requiredNumbers) {
    const summary = writeSkippedScenario(
      SCENARIO,
      ["C2", "C5"],
      `blocked before send: ${rps} RPS with ${env.cooldownSeconds}s cooldown requires ${requiredNumbers} allowed test numbers; only ${numbers.length} are configured`,
    );
  ...
  if (!budget.canReserve(totalRequests)) {
    const summary = writeSkippedScenario(
      SCENARIO,
      ["C2"],
      `remaining budget ${budget.remaining} is below required ${totalRequests}`,
    );
    printSummary(summary);
    return;
  }
  ...
  for (let i = 0; i < totalRequests; i += 1) {
    ...
    inflight.push(
      sendOtp(client, SCENARIO, phone, budget, `burst index ${i}`).then((result) => {
        results.push(result);
      }),
    );
  ...
  const status: ScenarioStatus = successRate >= 0.98 && !has5xx && p95 <= 5000 ? "GREEN" : "RED";
  const summary = writeScenarioSummary(
    ...
    ["C2", "C5"],
    status,
    `${(successRate * 100).toFixed(1)}% accepted, p95 ${p95}ms, 5xx=${has5xx}`,
  );
  printSummary(summary);
}
