...
export async function runT6() {
    ...
    if (!budget.canReserve(targets.length)) {
        const summary = writeSkippedScenario(SCENARIO, ["C4"], `remaining budget ${budget.remaining} is below abuse ceiling ${targets.length}`);
    ...
    for (const target of targets) {
        ...
        results.push(result);
        if (result.accepted) {
            const summary = writeScenarioSummary(SCENARIO, startedAt, results, ["C4"], "RED", `accepted abuse target in ${target.label}; stop immediately to avoid billing leakage`);
            printSummary(summary);
            process.exitCode = 1;
            return;
        }
    ...
    const status = results.every((result) => !result.accepted) ? "GREEN" : "RED";
    const summary = writeScenarioSummary(SCENARIO, startedAt, results, ["C4"], status, `${results.filter((result) => !result.accepted).length}/${results.length} abuse sends rejected`);
    printSummary(summary);
}
runT6().catch((error) => {
