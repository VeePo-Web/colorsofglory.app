...
async function waitUntil(startMs: number, seconds: number): Promise<void> {
  const target = startMs + seconds * 1000;
  const remaining = target - Date.now();
  if (remaining > 0) await sleep(remaining);
}

export async function runT5(): Promise<void> {
  ...
  if (!budget.canReserve(worstCase)) {
    const summary = writeSkippedScenario(
      SCENARIO,
      ["C5"],
      `remaining budget ${budget.remaining} is below required ${worstCase}`,
    );
  ...
  const status: ScenarioStatus =
    firstSuccessSeconds >= lowerBound && firstSuccessSeconds <= upperBound ? "GREEN" : "RED";
